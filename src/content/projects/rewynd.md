...
title: "Rewynd"
description: >-
  Built to fill a gap I felt while backtesting on FX Replay, and it ended up catching the attention of their team.
tags:
  - React
  - TypeScript
  - Chrome MV3
...
I got into trading the way many people do: some early wins that made me think I knew what I was doing. I didn't. The same leverage that grew a small account wiped most of it just as quickly, and that's when it clicked. Trading isn't really about prediction, it's about discipline, consistency, and managing risk.

To actually practice, I started using FxReplay to replay historical market data and trade it as if it were live. But two things kept bothering me: it didn't show how long a trade had been open, or the risk exposure per position. You had to track or calculate those yourself.

So I built a Chrome extension to surface those automatically. What started as a small quality-of-life fix eventually grew into a full analytics dashboard that turns replay sessions into simulated prop firm challenges, where traders operate under strict rules and get funded if they pass.

After I shared it on Reddit, the FxReplay team reached out and told me the extension had inspired them to build their own version of the feature.

## How it works

![Architecture diagram](/src/assets/projects/rewynd/architecture_diagram.svg)

FxReplay exposes no API, so the DOM is the only interface: the content script reads balance, PnL, and trade rows directly from the rendered HTML. A background service worker receives that data, persists it via Chrome's Storage API, and acts as the coordination layer between extension components. The dashboard, a React app with Zustand for state and Recharts for visualizations, reads from storage and stays in sync through Chrome's native storage events. When anything writes to storage, everything listening updates automatically.

The dashboard updates reactively on every storage change. Streak tracking treats break-even trades as neutral, and sessions, configs, and journal notes can all be exported and restored so nothing is tied to a single browser.
